Shader "nmSeashore/SeashoreParallax"
{
    Properties
    {
		...
		_InvIOR ("Inverse IOR", Float) = 0.7518796920776367	// 水のIOR 1.33より1.0 / 1.33
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			[domain("tri")]
			d2f domain(HS_CONSTANT_DATA_OUTPUT hsConst, const OutputPatch<v2f, 3> p, float3 bary : SV_DomainLocation)
			{
				UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(p[0]);
				UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(p[1]);
				UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(p[2]);
			...
			float4 frag(d2f i) : SV_Target
			{
				...
				float3 normal = waterSurfaceNormal(pos, beachMask * shore.z);
                float3 worldNormal = normalize(mul(float3x3(i.tspace0, i.tspace1, i.tspace2), normal));
				
				// 視差による地面の描画
				float3 plane = normalize(float3(_BeachSlope, 1.0, 0));
				float3 hitResult = ParallaxPlane(i.localPos + float3(0, 0, scrollTime()), viewDir, plane, worldNormal);
				col += float4(tex2D(_GroundTex, TRANSFORM_TEX(hitResult.xy, _GroundTex)).rgb * (1 - shore.w * (1 - _WetGroundTint.rgb) * _WetGroundTint.a), 1);
				
				// 泡の影
				#if FORM_SHADOW
				float2 formShadow = formMask(hitResult.xy);
				float4 shoreShadow = shoreline(hitResult.xy);
				shoreShadow = step(hitResult.x, 0) * shoreShadow + step(0, hitResult.x) * float4(formShadow, 0, 0);
				
				float frontShadowTex = shoreShadow.y * 2;
				float backShadowTex = shoreShadow.x;
				float shadowMask = frontShadowTex + lerp(backShadowTex, 0, saturate(frontShadowTex));
				float shadowTex = tex2D(_FormMask, hitResult.xy * 0.03333333 + normal.xy * 0.01).x;
				float shadowBorderRamp = max(fwidth(shadowTex), max(pos.x * 0.02, 0.5));
				
				col = lerp(col, 0, saturate(smoothstep(shadowMask + shadowBorderRamp * saturate(1 - POW2(shadowMask * 2 - 1)), shadowMask - shadowBorderRamp, shadowTex)) * POW2(saturate(lerp(1, 0, pos.x / breakerOffset(1)))));
				#endif
				
				// 深度に応じた色減衰
				col *= tex2D(_WaterGradient, float2(hitResult.z * _AttenuationScale, 0));
